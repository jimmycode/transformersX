...
def find_best_thresh_v2(preds, scores, na_probs, qid_to_has_ans):
    num_no_ans = sum(1 for k in qid_to_has_ans if not qid_to_has_ans[k])
    cur_score = num_no_ans  # assuming it answers all NoAnswer questions correctly
    best_score = cur_score
    best_thresh = 0.0
    qid_list = sorted(na_probs, key=lambda k: na_probs[k])  # sort qid by ascending na_probs
    for i, qid in enumerate(qid_list):
        if qid not in scores: continue
        # Assume the threshold is current na_probs, it will give HasAnswer
        if qid_to_has_ans[qid]:
            diff = scores[qid]
        else:
            if preds[qid]:
